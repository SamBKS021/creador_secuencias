  ...
  async function handleSubmit() {
    ...
    if (!nextName) {
      sileo.warning({
        ...
        description: "Escribe un nombre de categoría antes de guardarla.",
      });
      return;
    }
    ...
    try {
      if (editingName) {
        ...
        await actions.saveSongCategories(categories.map((category) => (category === editingName ? nextName : category)));
        ...
        sileo.success({
          ...
          description: affectedSongs.length
            ? `La categoría se actualizó en el catálogo y en ${affectedSongs.length} canto(s).`
        ...
        sileo.success({
          ...
          description: `${nextName} ya está disponible para tus cantos.`,
        });
      }
  ...
  async function handleDelete(category) {
    if (categories.length <= 1) {
      sileo.warning({
        ...
        description: "Debes conservar al menos una categoría disponible.",
      });
      return;
    }
    ...
    if (usageCount > 0) {
      sileo.warning({
        ...
        description: `Esta categoría está asignada a ${usageCount} canto(s). Cambia esos cantos antes de eliminarla.`,
      });
      return;
    }
    ...
    try {
      ...
      sileo.success({
        ...
        description: `${category} se eliminó del catálogo.`,
      });
    } catch (error) {
      sileo.error({
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <PageHeader
          eyebrow="Ajustes"
          title="Categorías"
          description="Crea, renombra y elimina categorías del catálogo general. Los cambios impactan biblioteca y Centro de carga."
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <EditorialCard className="space-y-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-headline text-2xl font-extrabold text-[var(--primary)]">Catálogo actual</h2>
              <p className="mt-1 text-sm text-[var(--on-surface-variant)]">Gestiona las categorías disponibles para todos tus cantos.</p>
            </div>
          </div>
            ...
            {categories.map((category) => (
              <div key={category} className="rounded-[24px] bg-[var(--surface-container-low)] px-5 py-5">
                <div className="space-y-4">
                  <div>
                    <h3 className="font-headline text-2xl font-extrabold text-[var(--primary)]">{category}</h3>
                    <p className="mt-2 text-sm leading-6 text-[var(--on-surface-variant)]">
                      {usageMap[category] || 0} canto(s) usando esta categoría
                    </p>
                  </div>
        ...
        <EditorialCard className="space-y-5">
          <div>
            <h2 className="font-headline text-2xl font-extrabold text-[var(--primary)]">
              {editingName ? "Editar categoría" : "Nueva categoría"}
            </h2>
            <p className="mt-2 text-sm leading-7 text-[var(--on-surface-variant)]">
              {editingName
                ? "Si renombras una categoría, también se actualizará en los cantos que ya la usan."
                : "Agrega una nueva categoría para que aparezca en formularios y filtros."}
            </p>
          </div>

          <label className="space-y-2">
            <span className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--outline)]">Nombre de la categoría</span>
            <input
          ...
          <div
            className="mt-3 rounded-[20px] border px-4 py-4 text-sm leading-6"
            style={{
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <p>Antes de eliminar una categoría, asegúrate de que ya no esté asignada a cantos existentes.</p>
            </div>
          </div>

          <div className="flex flex-wrap items-center justify-end gap-2">
            {editingName ? (
              <Button variant="ghost" className="rounded-lg px-3 py-2 text-xs" onClick={resetForm} disabled={saving}>
                <X size={14} />
                Cancelar
              </Button>
            ) : null}
            ...
            <Button className="rounded-lg px-3 py-2 text-xs" onClick={handleSubmit} disabled={saving}>
              ...
              {editingName ? "Guardar cambios" : "Agregar categoría"}
            </Button>
          </div>
        </EditorialCard>
      </div>
