import { CalendarDays, FileStack, FolderPlus, Upload } from 'lucide-react'
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 xl:grid-cols-4">
        ...
        <StatCard label="Nueva secuencia" value="Crear" accent="primary" icon={FolderPlus} />
        <StatCard label="Subir canción" value="Importar" accent="secondary" icon={Upload} />
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[2fr_1fr]">
        ...
        <div className="space-y-6">
          <EditorialCard>
            ...
            <div className="mt-5 space-y-4">
              {state.stats.upcomingSequences.map((sequence) => (
                <div key={sequence.id} className="rounded-2xl bg-[var(--surface-container-low)] p-4">
                  ...
                  <h3 className="font-headline mt-2 text-xl font-extrabold text-[var(--primary)]">{sequence.title}</h3>
            ...
            <Button className="mt-6 w-full" variant="outline" onClick={() => navigate('/constructor-secuencias')}>
              Abrir constructor
            </Button>
          </EditorialCard>

          <EditorialCard className="space-y-4">
            <p className="font-headline text-xs font-bold uppercase tracking-[0.24em] text-[var(--outline)]">Carga reciente</p>
            {state.stats.recentUploads.slice(0, 2).map((song) => (
              <div key={song.id} className="flex items-center gap-3 rounded-2xl bg-[var(--surface-container-low)] p-4">
